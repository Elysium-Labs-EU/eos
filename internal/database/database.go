...
// SQLite "attempt to write a readonly database (8)" (see issue #14). See
// ownership.Align for the shared self-healing logic, also used for the
// daemon's log dir/file and PID file (issue #91).
func alignDataFileOwnership(baseDir, dbPath string) error {
	...
	return ownership.Align(baseDir, dbPath, dbPath+"-wal", dbPath+"-shm")
}
