...
// fix for issue #91: when newStandaloneDaemon runs as root, the PID file, the
// log dir/file, and the control socket it creates are all chowned to match
// baseDir's owner rather than being left root-owned — the same self-healing
// behavior alignDataFileOwnership already gives state.db (issue #14). The
// socket assertion covers issue #306, where the socket was found root-owned
// on a real host despite the pidfile/log fix already being in place.
// PIDFile/LogDir/SocketPath deliberately live under sockDir here, separate
// from baseDir (dbDir), to prove ownership is aligned to baseDir's owner
// regardless of where those paths sit relative to it.
