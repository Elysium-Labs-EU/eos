...
// record and processHistory shows a still-live process for it. A recorded
// instance with nothing alive in its history is stale (e.g. the daemon was
// killed out-of-band without a clean `eos stop`) and is not an error: the
// caller self-heals by proceeding with the start.
